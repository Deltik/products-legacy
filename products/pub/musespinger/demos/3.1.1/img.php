...
// 2014-05-29 Compatibility Patch
error_reporting(0);
...
// Draw the image
	...
	$online = imagecreatefrompng($online);// Create the image resource for the online image
	$offline = imagecreatefrompng($offline);// Create the image resource for the offline image
	...
	if($cust != 1) {// If the image is not custom
		imagepng($im, $im_file, 0, NULL);// Save the image to disk
	}
