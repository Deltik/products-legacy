...
function process()
  ...
  if (type == "image")
    ...
    document.getElementById("result").innerHTML = '<img src="img.php?' + data + '&time=' + unixtime + '" alt="Loading status image...  If the image does not appear within <?=ini_get('max_execution_time')?> seconds, then the image could not be loaded." />';
    }
  else
    {
    var dataarray = document.getElementById("data").value.split(" ");
