...
// Process Cleverbot
//  Usage: processCleverbot([SESSION_ID, DATA_TO]);
function processCleverbot($session_id = false, $message = false)
  {
  // TODO
...
// Process Sensation Bot
//  Usage: processSensationbot([SESSION_ID, DATA_TO, MODE]);
function processSensationbot($session_id = false, $message = false, $mode = "general")
  {
  // TODO
  // Log the sending time
  $sent_time = date("d F Y h:i:s A");
  ...
  if (!$session_id)
    $session_id = md5(uniqid("aigui-"));
  
  // Set the data to be sent
  $postdata = http_build_query(
    array(
      "line" => stripslashes($message),
      )
    );
  // Hey robot... Remember me?
  $opts = array(
  ...
  // Alright, PHP... Make it happen
  $reply_raw = file_get_contents("http://www.sensationbot.com/jswrap.php?db=$mode&chatid=$session_id&pd=2000&kp=1&foc=1", false, $context);
  ...
  // Convert the conversation to AIGUI
  $reply_proc[] = "mi> ".stripslashes($message);
  $reply_proc[] = "yu> ".trim($reply_raw);
  
  // Make AIGUI headers
  array_unshift($reply_proc, $session_id, "$sent_time", "$reply_time", "--\n");
