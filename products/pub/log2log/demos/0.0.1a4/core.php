...
// Version Number
autodefine("LOG2LOG_VERSION", "0.0.1a4", true);
...
class Core
    ...
    if ($_FILES['file']['error'] == 0 || /*XXX*/$_FILES['file']['error'] == 4/*XXX*/)
      ...
      if (!$archive_type)
        ...
        foreach ($archive->files as $key => $info)
          ...
          log2log_debug_info("core", "Created directory structure path: ".$info['name']);
          }
  ...
  public function export($log_export)
    ...
    // Append Conversion Logs, if requested
    if ($_REQUEST['add_log'] == "yes")
      {
      chdir($tmp);
      @mkdir("log2log-debug", 0777, true);
      chdir("log2log-debug");
      global $LOG2LOG_info, $LOG2LOG_warning, $LOG2LOG_error;
      file_put_contents("log2log-debug-info.txt", var_export($LOG2LOG_info, true));
      file_put_contents("log2log-debug-warning.txt", var_export($LOG2LOG_warning, true));
      file_put_contents("log2log-debug-error.txt", var_export($LOG2LOG_error, true));
      chdir($tmp);
      $archive->addFile("log2log-debug/log2log-debug-info.txt");
      $archive->addFile("log2log-debug/log2log-debug-warning.txt");
      $archive->addFile("log2log-debug/log2log-debug-error.txt");
      }
...
function log2log_debug_info($executor, $data)
  ...
  # NOTE: Debugging script is not useful in production releases of Log2log.
  #       This code is safe to remove. If this is a production release of
  #       Log2Log, then it /should/ have already been removed.
  # Extra debug processing script
  if (file_exists("debug.php")) @include ("debug.php");
  }
