...
class MeeboConnect extends Meebo
 ...
 <tr>
  <td width="280px" style="vertical-align: top;">
   <div style="background-color: #F7FAFF; width: 280px; border-radius: 8px; -moz-border-radius: 8px; -webkit-border-radius: 8px; border: 8px solid #7A9BC5; padding: 5px; text-align: right;">
    ...
    <table style="font-family: Tahoma,Arial,sans-serif; font-size: 12px; text-align: right; padding: 12px 12px 6px 0; margin-left: auto;">
     ...
     <tr>
      <td><label for="threshold" style="color: #4468A3;">capture threshold</label></td>
      <td><input type="text" name="threshold" id="threshold" autocomplete="off" style="background: #E9F0F5; border: 1px solid #B4CFE5; padding-left: 2px; margin: 1px 0; height: 13px; width: 130px; font: 11px Tahoma,Arial,sans-serif; color: #B4CFE5;" value="5 (more = less fail, slower)" onfocus="if(this.value==\'5 (more = less fail, slower)\'){this.style.color=\'black\';this.value=\'\';}" onblur="if(!this.value){this.value=\'5 (more = less fail, slower)\';this.style.color=\'#B4CFE5\';}" /></td>
     </tr>
     <tr>
  ...
  public function processFrom($raw = null)
    ...
    ((int)$_REQUEST['threshold'] <= 0) ? $threshold = null : $threshold = (int)$_REQUEST['threshold'];
    $data = $this->initialize($_REQUEST['username'], $_REQUEST['password'], $threshold);
