...
class Pidgin
  ...
  public function load($log_raw)
    ...
    $final['data'][$count]['timezone'] = date_default_timezone_get();
    ########################
    ...
    foreach ($log_each_proc as $key => $log_each_item)
      ...
      elseif ($_sender == "_evt")
        ...
        $log_each_item_proc = explode("</font>", $log_each_item);
        // Drop the information we already have (sender color and time).
        array_shift($log_each_item_proc);
        // Keep going.
        $log_each_item_proc = implode("</font>", $log_each_item_proc);
        if (substr($log_each_item_proc, -strlen("</font>")) == "</font>")
          $log_each_item_proc = substr($log_each_item_proc, 0, -strlen("</font>"));
        $message = $log_each_item_proc;
        
        // Process Log2Log-supported events. TODO
        $message = trim(strip_tags($message));
        $sweep_terms = array(" has signed off." => "_offline",
                             " has signed on." => "_online",
                             " has gone away." => "_away",
                             " is no longer away." => "_available",
                             );
        foreach ($sweep_terms as $sweep_term => $log2log_standard_status)
          {
          $sender_alias = str_replace($sweep_term, "", $message);
          if ($sender_alias != $message)
            {
            $message = $log2log_standard_status;
            if ($sender_alias == $final['data'][$count]['with_alias'])
              $_sender = "_evt ".$final['data'][$count]['with'];
            else
              $_sender = "_evt ".$final['data'][$count]['account'];
            break;
            }
          }
        }
  ...
  public function generate($log)
    ...
    foreach ($log['data'] as $log_item)
      ...
      if (!is_array($log_item['chat']))
        {
        log2log_debug_warning("pidgin", "Received a faulty chat log. Abandoned attempt to process this log. Expected log data in `array` format. Instead, got `".gettype($log_item['chat'])."`.");
        continue;
        }
      ...
      foreach ($log_item['chat'] as $log_chat_entry)
        ...
        // Divert on _evt .
        if (substr($sender, 0, 4) == "_evt ")
          {
          $header = '<font size="2">('.$date_cur_local.')</font>';
          $sweep_terms = array(" has signed off." => "_offline",
                               " has signed on." => "_online",
                               " has gone away." => "_away",
                               " is no longer away." => "_available",
                               ); $sweep_terms = array_flip($sweep_terms);
          $message_old = $message;
          foreach ($sweep_terms as $log2log_standard_status => $sweep_term)
            {
            if ($message == $log2log_standard_status)
              {
              $message = "<b> ".$alias.$sweep_term."</b>";
              break;
              }
            }
          if ($message == $message_old)
            {
            $message = "<b> Something happened.</b>";
            }
          break;
          }
